...
contract MultiSig {
  ...
  constructor(uint256 _chainId, address[] memory _owners, uint _signaturesRequired) payable {
  ...
  function updateSignaturesRequired(uint256 newSignaturesRequired) public onlySelf {
    require(newSignaturesRequired>0,"updateSignaturesRequired: must be non-zero sigs required. fail.");
    signaturesRequired = newSignaturesRequired;
  }
    ...
    returns (bytes memory)
  {
    ...
    for (uint i = 0; i < signatures.length; i++) {
      ...
      require(recovered!=duplicateGuard, "executeTransaction: duplicate or unordered signatures. fail.");
    ...
    require(validSignatures>=signaturesRequired, "executeTransaction: not enough valid signatures. fail.");
